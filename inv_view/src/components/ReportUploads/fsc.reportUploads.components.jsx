...
const FscReportUpload =({
  header, dataTracked, uploadType, url, dupeCheck, dupeDelete, 
  lastUpdatedUrl, newestRecordUrl, lastUpdatedPostUrl}) => {
        ...
        await Promise.all([
          (async() => {
            ...
            if (header === 'Adjustment Report Upload' || header === 'Cycle Count Report Upload'){
              ...
              let getDupeCheck = await axios.get(`${FSC_API_URL}fsc/data${dupeCheck}`, {cancelToken: source.token});
              setDupes(getDupeCheck.data)

              let getNewestRec = await axios.get(`${FSC_API_URL}fsc/data${newestRecordUrl}`, {cancelToken: source.token});
            ...
            let getLastUpdates = await axios.get(`${FSC_API_URL}fsc/data${lastUpdatedUrl}`, {cancelToken: source.token});
    ...
    const onFileUpload = async() => {
      ...
      try{
        ...
        await Promise.all([
          ...
          await axios.post(`${FSC_API_URL}fsc/excel${url}`, formData, {cancelToken: source.token})
          .then(res => { alert(res.data.message) });
          
          await axios.post(`${FSC_API_URL}fsc/data${lastUpdatedPostUrl}`, {cancelToken: source.token})
    ...
    const deleteDuplicates = () =>{
      axios.delete(`${FSC_API_URL}fsc/data${dupeDelete}`)
  ...
  export default FscReportUpload;